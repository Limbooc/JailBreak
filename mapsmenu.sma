...
public plugin_init()
{
	...
	register_clcmd("say /cancelmaps", "cancelmaps", ADMIN_VOTE, "- cancelmaps")
...
public autoRefuse()
{
	...
	UTIL_SayText(0, "!y[!gMaps] !yРезультат не принят");
	if(ResultOk) ResultOk = false;
	g_iBitUserMapsIni = 0;
}
...
public actionResult(id, key)
{
	...
	switch (key)
	{
		case 0:
		{
			...
			formatex(NextMaps, charsmax(NextMaps), "%s", tempMap);
			
			UTIL_SayText(0, "!y[!gMaps] !yРезультат принят, следующая карта !g%s", NextMaps);
			UTIL_SayText(0, "!y[!gMaps] !yИграем последний !gраунд!y!");
			
			SetBit(g_iBitUserMapsIni, id);
		}
		case 1: autoRefuse()
	}
...
public checkVotes(id)
{
	...
	if (g_choosed != -1)
	{
		...
		UTIL_SayText(0, "!y[!gMaps] !yГолосование не состоялось");
...
public voteCount(id, key)
{
	if (key > 3)
	{
		...
		UTIL_SayText(0, "!y[!gMaps] !yГолосование отменено");
		remove_task(34567 + id)
	...
	if (get_cvar_float("amx_vote_answers"))
	{
		...
		new tempMap[32];
		ArrayGetString(g_mapName, key + 2, tempMap, charsmax(tempMap));
		
		
		new iPercent = percent(g_voteCount[key], g_TotalVotes);
		
		UTIL_SayText(0, "!t[!gMaps!t] !y%s проголосовал за !g%s !y(!g%d - %d%%!y)", name, tempMap, g_voteCount[key], iPercent);
	}
...
displayVoteMapsMenu(id, pos)
{
	...
	if(ResultOk)
	{
		UTIL_SayText(id, "!t[!gMaps!t] !yДосрочное голосование закончена, следующая карта !g%s", NextMaps);
		UTIL_SayText(id, "!t[!gMaps!t] !yДля отмены смены карт, !gинициатор голосвание !yдолжен отменить смену карту (!g/cancelmaps!y)");
		return;
	}
...
public cancelmaps(id, level, cid)
{
	...
	if (get_cvar_float("amx_last_voting") > get_gametime())
	{
		...
		UTIL_SayText(id, "!y[!gMaps] !yГолосование уже начато...");
		return PLUGIN_HANDLED
	}
	if(IsNotSetBit(g_iBitUserMapsIni, id))
		return PLUGIN_HANDLED;
	
	g_iBitUserMapsIni = 0;
	ResultOk = false;
	
	UTIL_SayText(0, "!t[!gMaps!t] !y!g%n !yотменил досрочное смены карт",id);
	return PLUGIN_HANDLED
}
...
public cmdVoteMapMenu(id, level, cid)
{
	...
	if (g_mapNums)
	{
		...
		UTIL_SayText(id, "!y[!gMaps] !yВ меню нет карт");
	}
...
public actionVoteMapMenu(id, key)
{
	...
	switch (key)
	{
		case 7:
		{
			...
			UTIL_SayText(0, "!y[!gMaps] АДМИНИСТРАТОР !g%s: !yсоздал голосование за карту", name);
...
public NewRound() 
{
	if(ResultOk)
	{
		if(isValidMap(NextMaps))
		{
			...
			set_task(2.0, "delayedChange", 0, NextMaps, strlen(NextMaps) + 1)
			//delayedChange(NextMaps);
		}
	}
}
