...
#define MODEL_CLASSNAME "env_sprite"

//access level for the commands
#define ADMIN_ACCESS_LEVEL ADMIN_RCON
...
public plugin_init()
{
	...
	register_clcmd("+place_ad_dasdsa", "cmd_place_ad", ADMIN_ACCESS_LEVEL)
	register_clcmd("-place_ad_dasdsa", "cmd_place_ad", ADMIN_ACCESS_LEVEL)
	
	register_clcmd("delete_ad_dasdsa", "cmd_delete_ad", ADMIN_ACCESS_LEVEL)
	
	register_clcmd("iga_closer_dasdsa", "cmd_move_toward_client", ADMIN_ACCESS_LEVEL)
	register_clcmd("iga_farther_dasdsa", "cmd_move_away_from_client", ADMIN_ACCESS_LEVEL)
	register_clcmd("iga_further_dasdsa", "cmd_move_away_from_client", ADMIN_ACCESS_LEVEL)
...
{		
	...
	static iszInfo = 0;
	if(iszInfo || (iszInfo = engfunc(EngFunc_AllocString, "info_target"))) g_ent = engfunc(EngFunc_CreateNamedEntity, iszInfo);
...
public show_delete_icon(ent, show)
{
	...
	if(show)
	{
		...
		static iszInfo = 0;
		if(iszInfo || (iszInfo = engfunc(EngFunc_AllocString, "info_target"))) ent2 = engfunc(EngFunc_CreateNamedEntity, iszInfo);
